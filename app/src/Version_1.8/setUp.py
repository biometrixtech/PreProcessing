...
class Set_Up(object): #Abstract setUp class
    ...
    @abc.abstractmethod
    def __init__(self, path, columns, hz, anatom=None, calib=None):
        ...
        if calib is True:
            self.hipdataset = do.RawFrame(data[:,11:21], columns)
            self.lfdataset = do.RawFrame(data[:,1:11], columns)
            self.rfdataset = do.RawFrame(data[:,21:31], columns)
        else:    
...
class SensCal(object):
    def __init__(self, path, hz):
        columns = ['aX', 'aY', 'aZ', 'gX', 'gY', 'gZ', 'qW', 'qX', 'qY', 'qZ'] #declare columns to be passed
        Set_Up.__init__(self, path, columns, hz, calib=True) #create setUp object
