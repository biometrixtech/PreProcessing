...
def clippedAccel(acc):
    #test for clipped acceleration
    limits = [2000, -2000, 4000, -4000, 8000, -8000, 16000, -16000] #various sensitivities
    for i in range(len(limits)): #if accel values within 4 mGs of sensitivities mark as clipped
        if -4 < acc-limits[i] < 4: #
            return 'clipped' 
...
def TransformData(data):
    ...
    for i in range(iters):
        ...
        #check for quaternions that may be corrupted
        if .999 > np.linalg.norm(q0) or  np.linalg.norm(q0) > 1.001:
            #set t=i quat to t=i-1 quaternion
            q0 = np.matrix([data.qW[i-1], data.qX[i-1], data.qY[i-1], data.qZ[i-1]])
        ...
        if data.aZ[i] - data.aZ[i-1] == 0 and abs(data.aZ[i]) > 1900:
            out = clippedAccel(data.aZ[i])
        elif data.aX[i] - data.aX[i-1] == 0 and abs(data.aX[i]) > 1900:
            out = clippedAccel(data.aX[i])
        elif data.aY[i] - data.aY[i-1] == 0 and abs(data.aY[i]) > 1900:
            out = clippedAccel(data.aY[i])
