...
def test_placement_check(left_acc, hip_acc, right_acc, bad_left,
    ...
    if bad_left:
        assume (np.nanmean(left_acc[:, 0]) < 200 or
                np.nanmean(left_acc[:, 1]) > -200 or
                np.absolute(np.nanmean(left_acc[:, 2]) > 200))
        assert np.in1d(ind, [2, 3, 6, 7])
    if bad_hip:
        assume (np.absolute(np.nanmean(hip_acc[:, 0])) > 200 or 
                np.nanmean(hip_acc[:, 1]) > -800 or
                np.absolute(np.nanmean(hip_acc[:, 2])) > 200)
        assert np.in1d(ind, [2,3,4,5])
    if bad_right:
        assume (np.nanmean(right_acc[:, 0]) > -200 or
                np.nanmean(right_acc[:, 1]) > -200 or
                np.absolute(np.nanmean(right_acc[:, 2])) > 200)
