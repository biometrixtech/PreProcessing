# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 17:48:11 2016
...
length = 50
@given(arrays(np.float, length, elements=st.floats(min_value=-np.pi,
                                                   max_value=np.pi)),
       arrays(np.float, length, elements=st.floats(min_value=-np.pi,
                                                   max_value=np.pi)),
       arrays(np.float, length, elements=st.floats(min_value=-np.pi,
                                                   max_value=np.pi)),
       st.just(0))
...
def test_control_score(LeX, HeX, ReX, example):
    ...
    Tests with Example:
        ...
        -args(np.random.random(length), np.zeros(length), np.zeros(length) , 4)
            ...
            --For control_lf, control, ankle_control there are values
                other than 100  
    """
    ...
    control, hip_control, ankle_control, control_lf,control_rf =\
                                        control_score(LeX, HeX, ReX, ms_elapsed)
