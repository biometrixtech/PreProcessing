...
#                            +hip_acc_aif[i][2]**2)
    ...
    return accel_mag.reshape(len(hip_acc_aif),1)
...
#                print i + k, subset_data.shape
                if np.absolute(subset_data[k][1]) < np.pi/4: # I EDITED THIS: I + K TO K
...
def double_or_single_leg(lf_phase, rf_phase, standing, epoch_time):
    ...
    for x in islice(count(), 0, len(epoch_time), NMSEC_JUMP):
        ...
        for i in range(len(subset_data)):
            _doub_sum = 0
#            print len(subset_data), "sub data"
            # use _stand_sum as counter to see where in past window of
            # time subject has been standing on 2 legs
            for k in range(_double_win):
#                print _double_win, "doub win"
#                print len(_lf_phase_iso_stand), "_lf_phase_iso_stand"
                if _lf_phase_iso_stand[k].item() == 1 and \
                    _rf_phase_iso_stand[k].item() == 1: # EDITED I + K TO K FOR TESTING
                    _doub_sum = _doub_sum + 1
                   
                   # subject has been double leg standing for duration
                   # of window, assume standing at that time
...
#                    print 'code sing', single_leg
                # subject not double leg standing but has at least 1 foot on
                # ground, so single leg standing
                elif (_lf_phase_iso_stand[k].item() in [2, 3, 5, 6] or \
                    _rf_phase_iso_stand[k].item() in [2, 3, 5, 6]): # EDITED I + K TO K FOR TESTING
                    single_leg[i] = 1
#                    print 'code sing', single_leg
                elif (double_leg[i] != 0 and single_leg[i] != 0):
                    feet_eliminated[i] = 1
    ...
    for x in islice(count(), 0, len(epoch_time), NMSEC_JUMP):
        ...
        for i in range(len(subset_data)):
            ...
            for k in range(_stationary_win):
                ...
                # left leg analysis
                if _lf_phase_iso_sing[k].item() == 2: # EDITED I + K TO K FOR TESTING
                ...
                # right leg analysis
                elif _rf_phase_iso_sing[k].item() == 3: # EDITED I + K TO K FOR TESTING
                    _stat_sum = _stat_sum + 1
