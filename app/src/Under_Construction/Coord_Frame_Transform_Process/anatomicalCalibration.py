...
#    hip_asf_transform = qo.quat_prod(np.array(([0.70710678,0.70710678,0,0])),
#                                     np.array(([0.70710678,0,0,0.70710678])))
    # FOR OLD SENSORS RUNNING SIDE TO SIDE (sensor on right)
#    hip_asf_transform = np.array(([0.70710678,0,-0.70710678,0]))
...
#    rot_y = np.array([np.sqrt(.5), -np.sqrt(.5), 0, 0])[np.newaxis, :]
#    rot_x = np.array([np.sqrt(.5), 0, np.sqrt(.5), 0])[np.newaxis, :]
