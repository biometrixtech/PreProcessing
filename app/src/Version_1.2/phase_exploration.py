...
def Move(std, w, new_u, new_std, pitch): #inputs array of st. devs    
    if std > new_u + 1*new_std and std >1.2: #if data point exceeds 1 SD and is great than 1.2
        return 1 #add to list
    else:
        if pitch >= .9: #check pitch to decide if foot is off ground but not moving
            return 1 #add to list
        else:
            return 0 #add to list

def Grad_Move(u, w, new_u, new_std, pitch): #inputs array of data points 
