...
def _phase_detect(acc, hz):
    ...
    Returns:
        bal_phase: a numpy array that returns 1's and 0's for foot in the air and foot on the ground respectively
...
def _bound_det_lf(p):
    ...
    Returns:
        start_move: an array that stores the indexes when the 'foot in the air' phase begins for the left foot
        end_move: an array that stores the indexes when the 'foot in the air' phase ends for the left foot
...
def _bound_det_rf(p):
    ...
    Returns:
        start_move: an array that stores the indexes when the 'foot in the air' phase begins for the right foot
        end_move: an array that stores the indexes when the 'foot in the air' phase ends for the right foot
...
def _impact_detect(start_move, end_move, az, hz):
    ...
    Args:
        start_move: an array of the indexes when the 'foot in the air' phase begins for left/right foot
        end_move: an array of the indexes when the 'foot in the air' phase ends for left/right foot
        az: vertical acceleration of left/right foot
        hz: sampling rate
        
    Returns:
        imp: a 2d array that stores the indexes of when the impact phase begins and ends for left/right foot
...
def combine_phase(laccz, raccz, hz):
    ...
    """Combines the balance, foot in the air and the impact phases for the left and right feet.
    ...
    ph = _body_phase(raccz, laccz, hz) #balance phase for both the right and left feet  
    
    lf_ph = list(ph)
    rf_ph = list(ph)
    
    lf_sm, lf_em = _bound_det_lf(lf_ph) #detecting the start and end points of the left foot movement phase
    rf_sm, rf_em = _bound_det_rf(rf_ph) #detecting the start and end points of the right foot movement phase 
    
    lf_imp = _impact_detect(lf_sm, lf_em, laccz, hz) #starting and ending point of the impact phase for the left foot
    rf_imp = _impact_detect(rf_sm, rf_em, raccz, hz) #starting and ending points of the impact phase for the right foot
