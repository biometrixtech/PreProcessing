    ...
    def __init__(self, sensor_data, file_name):
        ...
        try:
            ...
            if exercise_id is None:
                ...
                logger.warning("exercise_id associated with file not found")
        ...
        try:
            ...
            logger.warning("Associated session_anatomical calibration event \
                            not found")
            raise error
        ...
        if len(hip_n_transform) == 0:
            ...
            raise ValueError("Missing Offests")
        ...
        if len(self.data.HaX) < 2000:
            ...
            try:
                ...
                logger.warning("Cannot write to s3 after success!")
                raise error
            try:
                cur.execute(quer_rpush, (user_id, msg, r_push_data))
                conn.commit()
                ...
                logger.warning("Cannot write to rPush after success!")
                raise error
