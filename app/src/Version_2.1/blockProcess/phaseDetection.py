...
def _phase_detect(acc, hz):
    ...
    Args:
        acc: foot acceleration in the adjusted inertial frame
    ...
    bal_win = int(0.08*hz)  # setting a sampling window to determine balance 
                            # phase
    dummy_balphase = []  # dummy variable to store the indexes of balance 
                            # phase

    # determine when balance phase occurs
    for i in range(len(acc) - bal_win):
        count = 0
        for j in range(bal_win):
            if abs(acc[i+j]) <= thresh:  # checking whether each data point in 
            # the sampling window is lesser than or equal to the thresh
                count = count + 1
        if count == bal_win:  # checking if the number of data points that are 
        # considered as "balance phase" equal the sampling window 
    ...
    min_thresh_mov = int(0.024*hz)  # a threshold for minimum number of samples 
    # required to be classified as a false movement phase
...
def _bound_det_lf(p):
    ...
    Returns:
        ...
        end_move: an array that stores the indexes when the 'foot in the air' 
        phase ends for the left foot
    ...
    for i in range(len(p)-1):
        if p[i] == phase_id.rflf_ground.value \
        and p[i+1] == phase_id.rf_ground.value:
            start_move.append(i+1)
        elif p[i] == phase_id.lf_ground.value \
        and p[i+1] == phase_id.rf_ground.value: 
            start_move.append(i+1)
        elif p[i] == phase_id.rflf_ground.value \
        and p[i+1] == phase_id.rflf_offground.value: 
            start_move.append(i+1)
        elif p[i] == phase_id.lf_ground.value \
        and p[i+1] == phase_id.rflf_offground.value: 
            start_move.append(i+1)
        elif p[i] == phase_id.rf_ground.value \
        and p[i+1] == phase_id.rflf_ground.value:
            end_move.append(i)
        elif p[i] == phase_id.rf_ground.value \
        and p[i+1] == phase_id.lf_ground.value:
            end_move.append(i)
        elif p[i] == phase_id.rflf_offground.value \
        and p[i+1] == phase_id.rflf_ground.value: 
            end_move.append(i)
        elif p[i] == phase_id.rflf_offground.value \
        and p[i+1] == phase_id.lf_ground.value:
...
def _impact_detect(start_move, end_move, az, hz):
    ...
    # determine when impact occurs
    for i,j in zip(start_move, end_move):
        arr_len = []
        dummy_start_imp = []
        dummy_end_imp = []
        acc = az[i:j]  # acceleration values of the corresponding movement 
                        # phase
    ...
    # create a 2D list of start and end positions of impact phase            
    imp = []
...
def combine_phase(laccz, raccz, hz):
    ...
    """
    Combines the balance, foot in the air and the impact phases for the left 
    and right feet.
    ...
    # assigning the phase variable to a right & left feet variables
    lf_ph = list(ph)
    rf_ph = list(ph)
    
    # determine the start and end positions of 'foot in the air' phase
    lf_sm, lf_em = _bound_det_lf(lf_ph)  # detecting the start and end points 
                                            # of the left foot movement phase
    rf_sm, rf_em = _bound_det_rf(rf_ph)  # detecting the start and end points 
                                            # of the right foot movement phase 
    
    # determine the start and end positions of impact
    ...
    # check if impacts exist for left foot phase, if True add to left foot
    # phase variable
    ...
    # check if impacts exist for right foot phase, if True add to right foot
    # phase variable
