    ...
    def _get_movement_asymmetry(self, complexity_level, cma_level, grf_level, stance, left_steps, right_steps):
        ...
        for unit_block_number in unit_block_list:
            ...
            for i in range(0, intervals):
                ...
                l_step_blocks = list(x for x in l_unit_block_steps if start_time + (i * seconds) <= x.cumulative_end_time <= start_time + ((i + 1) * seconds))
                r_step_blocks = list(x for x in r_unit_block_steps if start_time + (i * seconds) <= x.cumulative_end_time <= start_time + ((i + 1) * seconds))
                time_block += 1
                event.anterior_pelvic_tilt_range = self._get_steps_f_test("anterior_pelvic_tilt_range", l_step_blocks, r_step_blocks, time_block)
                ...
                event.anterior_pelvic_tilt_rate = self._get_steps_f_test("anterior_pelvic_tilt_rate", l_step_blocks, r_step_blocks, time_block)
