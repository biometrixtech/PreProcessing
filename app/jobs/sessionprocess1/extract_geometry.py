...
import numpy as np
from utils.quaternion_operations import quat_prod, quat_conj
...
def extract_geometry(quats):
    ...
    v_k = quat_prod(quat_prod(quats.reshape(-1, 4), k_v), quat_conj(quats.reshape(-1, 4)))
    
    j_v = np.array([0, 0, 1, 0]).reshape(-1, 4)

    v_j = quat_prod(quat_prod(quats.reshape(-1, 4), j_v), quat_conj(quats.reshape(-1, 4)))
    
    i_v = np.array([0, 1, 0, 0]).reshape(-1, 4)

    v_i = quat_prod(quat_prod(quats.reshape(-1, 4), i_v), quat_conj(quats.reshape(-1, 4)))
