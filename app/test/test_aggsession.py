"""Testing functions within sessionAgg module
"""
import numpy
...
def test_ranges():
    ...
    res_data = agg_session._get_ranges(col_data, 1)
...
def test_contact_duration():
    ...
    dur_lf = agg_session._contact_duration(lf_phase, active, epoch_time, lf_ground)
    dur_rf = agg_session._contact_duration(rf_phase, active, epoch_time, rf_ground)
...
def test_duration_stats():
    ...
    record = agg_session._get_contact_duration_stats(length_lf, length_rf, record)
...
def test_peak_grf():
    ...
    peak_grf_lf, peak_grf_rf = agg_session._peak_grf(total_grf,
                                                     phase_lf,
                                                     phase_rf)
...
def test_peakgrf_stats():
    ...
    record = agg_session._get_peak_grf_stats(peak_grf_lf, peak_grf_rf, record)
